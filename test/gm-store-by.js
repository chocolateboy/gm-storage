...
const { GMStoreBy } = require('..')

Object.assign(global, API)

test.beforeEach(t => {
    const backingStore = initBackingStore()
    const store = new GMStoreBy({ key: JSON })
...
test('forEach', t => {
    ...
    store.forEach(function (value, key, $store) {
        t.is(this, undefined)
        t.is($store, store)
        t.true($store instanceof GMStoreBy)
        seen1.set(key, value)
    })

    t.deepEqual(Object.fromEntries(seen1), OBJECT)
    ...
    t.deepEqual(Object.fromEntries(seen2), OBJECT)
})
...
test('options.strict', t => {
    ...
    try {
        ...
        t.notThrows(() => new GMStoreBy({ strict: false, key: JSON }))

        t.throws(() => new GMStoreBy({ strict: true, key: JSON }), {
            instanceOf: ReferenceError,
            message: /GM_getValue is not defined/,
        })

        t.throws(() => new GMStoreBy({ key: JSON }), {
            instanceOf: ReferenceError,
            message: /GM_getValue is not defined/,
        })
