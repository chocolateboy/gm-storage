...
class GMStorage<K extends JSONValue = JSONValue, V extends JSONValue = JSONValue> implements Map<K, V> {
    ...
    public *entries (): Generator<[K, V]> {
        ...
        for (let i = 0; i < keys.length; ++i) {
            const key = keys[i]
            yield [parse(key), GM_getValue(key)!]
        }
    }
    ...
    public get (key: K, $default = undefined) {
        return GM_getValue(stringify(key), $default)
    }

    public has (key: K): boolean {
        return this._has(stringify(key))
    }
    ...
    public set (key: K, value: V): this {
        GM_setValue(stringify(key), value)
        return this
    }
...
interface GMStorage<K extends JSONValue = JSONValue, V extends JSONValue = JSONValue> extends Map<K, V> {
